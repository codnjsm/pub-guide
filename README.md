...
- `v-btn`에 `icon` prop을 주면서 default slot도 같이 넘기면(내용이 비어 있어도) Vuetify가 **슬롯을 우선**해서 아이콘이 렌더링되지 않는다. 아이콘 전용 버튼과 텍스트 버튼을 같은 곳에서 다뤄야 하면, 하나의 `v-btn`에 조건부로 섞지 말고 `v-if`/`v-else`로 태그 자체를 분리한다(`ButtonsView.vue` 참고).
